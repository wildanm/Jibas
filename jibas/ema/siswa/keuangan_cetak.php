...
<?
OpenDb();
$sql = "SELECT s.nama, k.kelas, t.tingkat FROM siswa s, kelas k, tingkat t WHERE ".
"s.nis = '$nis' AND s.idkelas = k.replid AND k.idtingkat = t.replid";
$result = QueryDb($sql);
$row = mysql_fetch_row($result);
$namasiswa = $row[0];
$kelas = $row[1];
...
$sql = "SELECT DISTINCT p.idpenerimaan, d.nama FROM $db_name_fina.penerimaaniuran p, $db_name_fina.datapenerimaan d WHERE p.idpenerimaan = d.replid AND p.nis='$nis' ORDER BY nama";
$result = QueryDb($sql);
while ($row = mysql_fetch_array($result)) {
