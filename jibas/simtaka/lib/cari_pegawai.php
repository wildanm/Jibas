...
if (isset($_REQUEST['submit']) || $_REQUEST['submit'] == 1) { 
	...
	if ((strlen($nama) > 0) && (strlen($nip) > 0)) {
		...
		$sql_tot = "SELECT nip, nama, bagian FROM  $db_name_sdm.pegawai WHERE aktif = 1 AND nip LIKE '%$nip%' $sql_tambahbag ORDER BY nama"; 		$sql_pegawai = "SELECT nip, nama, bagian FROM  $db_name_sdm.pegawai WHERE aktif = 1 AND nip LIKE '%$nip%' $sql_tambahbag ORDER BY $urut1 $urutan1 LIMIT ".(int)$page1*(int)$varbaris1.",$varbaris1";
	} 
