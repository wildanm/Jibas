...
class CPenulisEdit{
	...
	function OnStart(){
		if (isset($_REQUEST[simpan])){
			$sql = "SELECT kode FROM penulis WHERE kode='$_REQUEST[kode]' AND replid <> $_REQUEST[replid]";
			$result = QueryDb($sql);
			$num = @mysql_num_rows($result);
			if ($num>0){
				$this->exist();
			} else {
				$sql = "UPDATE penulis SET kode='$_REQUEST[kode]', nama='$_REQUEST[nama]', kontak='$_REQUEST[kontak]', biografi='$_REQUEST[biografi]', keterangan='$_REQUEST[keterangan]', gelardepan='$_REQUEST[gelardepan]', gelarbelakang='$_REQUEST[gelarbelakang]' WHERE replid=$_REQUEST[replid]";
				$result = QueryDb($sql);
				if ($result)
					$this->success();
			}
