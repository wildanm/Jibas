...
class CKatalogEdit{
	...
	function OnStart(){
		if (isset($_REQUEST[simpan])){
			$sql = "SELECT kode FROM katalog WHERE kode='$_REQUEST[kode]' AND replid <> $_REQUEST[replid]";
			$result = QueryDb($sql);
			$num = @mysql_num_rows($result);
			if ($num>0){
				$this->exist();
			} else {
				$sql = "UPDATE katalog SET rak='$_REQUEST[rak]', kode='$_REQUEST[kode]', nama='$_REQUEST[nama]', keterangan='$_REQUEST[keterangan]' WHERE replid=$_REQUEST[replid]";
				$result = QueryDb($sql);
				if ($result)
