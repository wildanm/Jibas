...
if (isset($_REQUEST['simpan']) && isset($_FILES[foto])){
		...
		//$updir = $G_UPLOADDIR . "media" . getOSSlash() . $thn . $bln . getOSSlash();
		//if (!is_dir($updir)) 
			//mkdir($updir, 0777);
		...
		$sql="INSERT INTO jbsvcr.gambartiny SET idguru='$pengguna', namagambar='$nama',bulan='$bln',tahun='$thn',keterangan='$keterangan' $gantifoto";
		//echo $sql;
		$result=QueryDb($sql);
		CloseDb();
		/*
		...
		$result_client=QueryDb($sql_client);
		$row_client=@mysql_fetch_array($result_client);
		$sql="INSERT INTO jbsvcr.gambartiny SET region='$row_client[region]',location='$row_client[location]',clientid='$row_client[clientid]',idguru='$idguru', namagambar='".$_REQUEST[nama]."',bulan=".$dir_bln.",tahun=".$dir_thn.",keterangan='$_REQUEST[keterangan]' $gantifoto";
		//echo $sql;
		$result=QueryDb($sql);
		CloseDb();
	*/
