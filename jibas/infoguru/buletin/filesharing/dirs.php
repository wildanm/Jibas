...
if ($op == "q836bzsda53cx5429h3cs52")
{	
	// Kalo EOF Begin jgn di apapain..............
	...
	if (@mysql_num_rows($result_file) > 0)
	{
	   ?><script language="javascript">
		 if (confirm('Direktori tidak kosong, \nAnda yakin akan menghapus Direktori ini?')){
		   return true;
		 } else {
	...
	$dirpath=str_replace("root/",$UPLOAD_DIR."fileshare".GetOSSlash(),$dirpath);
	//echo "1".$dirpath;
	if (is_dir($dirpath))
		deldir($dirpath);
		//rmdir($dirpath);
...
function renamefolder(iddir){
	newWindow('editfolder.php?iddir='+iddir,'UbahFolder','554','183','resizable=1,scrollbars=0,status=0,toolbar=0');
}
	...
	while ($row = mysql_fetch_row($result)) {
		...
		if ($nsubdir == 0) {
			...
			if (SI_USER_ID()==$idguru){
				...
				//if ($dirname!=$idguru)
				//	echo "<img onclick='renamefolder($iddir)' src='../../images/ico/ubah.png'>&nbsp;";
				if ($msg=="")
...
//onMouseOver=\"showhint('$ajar', this, event, '100px')\" title='Mengajar B. Indonesia, Matematika'
/*$sql_get_id = "SELECT pel.nama as pelajaran FROM jbsakad.pelajaran pel, jbsakad.guru g WHERE g.nip='$idguru' AND g.idpelajaran=pel.replid";
		$res_get_id	= QueryDb($sql_get_id);
