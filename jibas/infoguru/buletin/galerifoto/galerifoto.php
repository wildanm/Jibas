...
if ($op=="14075BUSYCODACALLDIFF"){
	...
	if (file_exists($GALLERY_DIR."photos\\".$r[filename])&& file_exists($GALLERY_DIR."thumbnail\\".$r[filename])){ // && !is_empty_dir($GALLERY_DIR."thumbnail\\") && is_empty_dir($GALLERY_DIR."photos\\")){
...
if ($op=="DIFFBUSYCODACALL14077"){
	OpenDb();
	$sql = "UPDATE jbsvcr.galerifoto SET nama='$_REQUEST[newName]',keterangan='".CQ($_REQUEST['newKet'])."' WHERE replid='$_REQUEST[replid]'";
	//echo($sql);
	QueryDb($sql);
	CloseDb();
	?>
...
<a href="../../home.php"  target="framecenter">Home</a> > <strong>Galeri Foto</strong><br /><br /></td>
	...
	<a href="galerifoto_ss.php"><img src="../../images/ico/filmstrip.gif" border="0">&nbsp;Filmstrip</a>&nbsp;&nbsp;
